...
const TRANSCODABLE_EXT = new Set([
	".svg",
	".tiff",
	".tif",
	".heic",
...
const NO_MAGICK = "this image format requires ImageMagick (magick); install it, or use PNG/JPEG/GIF/WebP/BMP";
/**
...
*/
async function transcodeToPng(ctx, bytes, signal) {
	const subprocess = ctx.get("subprocess");
	if (subprocess === void 0) throw new Error(`${NO_MAGICK} (and the subprocess service)`);
	...
	try {
		...
		if ((await handle.done).exitCode !== 0) {
...
async function resolveMagick(subprocess, signal) {
	try {
		return await subprocess.resolveExecutable("magick", void 0, signal);
	} catch {
		let convert;
		try {
			convert = await subprocess.resolveExecutable("convert", void 0, signal);
		} catch {
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (nativeMime === void 0 && !isTranscodable(args.path)) throw new Error(`cannot describe "${args.path}": only image files are supported (PNG/JPEG/GIF/WebP/BMP natively, or SVG/TIFF/HEIC/PSD/ICO/... when ImageMagick is installed)`);
			...
			const imageBytes = nativeMime === void 0 ? await transcodeToPng(ctx, bytes, exec.signal) : bytes;
